        ...
        function convert() {
            ...
            if (!octets) {
                if (ipInput.value.trim()) {
                    showError('Invalid IPv4 address. Use format: 0-255.0-255.0-255.0-255');
                }
            ...
            if (cidr) {
                ...
                document.getElementById('network-bits').textContent = prefix + ' bits';
                document.getElementById('host-bits').textContent = (32 - prefix) + ' bits (' + (Math.pow(2, 32 - prefix) - 2) + ' usable hosts)';
                ...
                visual += '<br><span class="text-xs mt-1 inline-block"><span class="text-gold">Network bits (gold)</span> · <span class="text-light-muted/50">Host bits (dimmed)</span></span>';
            ...
            showSuccess('Converted ' + ipInput.value.trim() + ' to binary');
        }
