...
<div class="mb-6 p-4 bg-darkBg rounded-lg border border-gold/10">
    <div class="grid sm:grid-cols-2 gap-4 mb-4">
        <div>
            <label for="quote-style" class="text-light font-semibold mb-2 block text-sm">{{ __($t . '.quote_style_label') }}</label>
            <select id="quote-style" class="w-full bg-darkCard border border-gold/20 rounded-lg px-3 py-2 text-light text-sm focus:border-gold/50 focus:outline-none cursor-pointer">
                <option value="smart">{{ __($t . '.quote_smart') }}</option>
                <option value="all">{{ __($t . '.quote_all') }}</option>
                <option value="never">{{ __($t . '.quote_never') }}</option>
            </select>
        </div>
    </div>
    <div class="flex flex-wrap gap-6">
        <label class="flex items-center gap-2 cursor-pointer">
            ...
            <span class="text-sm text-light-muted">{{ __($t . '.trim_whitespace') }}</span>
        </label>
    </div>
</div>
...
<div id="stats-bar" class="hidden bg-darkBg rounded-lg p-4 border border-gold/10">
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="text-center">
            ...
            <div class="text-light-muted text-sm">{{ __($t . '.stat_fields_quoted') }}</div>
        </div>
        <div class="text-center">
