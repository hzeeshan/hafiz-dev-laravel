...
<div class="bg-darkBg rounded-lg border border-gold/10 p-4 mb-6">
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {{-- Transaction Type --}}
        <div>
            <label for="opt-transaction-type" class="block text-sm text-light-muted mb-1">{{ __($t . '.transaction_type') }}</label>
            <select id="opt-transaction-type" class="w-full bg-darkBg border border-gold/20 rounded-lg px-3 py-2 text-light text-sm focus:border-gold focus:outline-none">
                <option value="personal_free">{{ __($t . '.personal_free') }}</option>
                <option value="personal_credit">{{ __($t . '.personal_credit') }}</option>
                <option value="business">{{ __($t . '.business') }}</option>
                <option value="instant_transfer">{{ __($t . '.instant_transfer') }}</option>
            </select>
        </div>
        ...
        <div>
            ...
            <select id="opt-mode" class="w-full bg-darkBg border border-gold/20 rounded-lg px-3 py-2 text-light text-sm focus:border-gold focus:outline-none">
                <option value="fee_from_amount">{{ __($t . '.fee_from_amount') }}</option>
                <option value="amount_to_receive">{{ __($t . '.amount_to_receive') }}</option>
            </select>
        </div>
    </div>
</div>
...
<div class="mb-6">
    <label for="input-amount" class="text-light font-semibold mb-2 flex items-center gap-2">
        ...
        <span id="input-label">{{ __($t . '.payment_amount') }}</span>
    </label>
    <div class="relative">
    ...
    <div class="bg-darkBg border border-gold/10 rounded-lg p-6 mb-4">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="text-center">
                <div class="text-xs text-light-muted mb-2 uppercase tracking-wider" id="result-label-1">{{ __($t . '.you_send') }}</div>
                <div class="text-3xl md:text-4xl font-bold text-gold font-mono" id="result-send">$0.00</div>
            </div>
            <div class="text-center">
                <div class="text-xs text-light-muted mb-2 uppercase tracking-wider">{{ __($t . '.venmo_fee') }}</div>
                <div class="text-3xl md:text-4xl font-bold text-red-400 font-mono" id="result-fee">-$0.00</div>
            </div>
            <div class="text-center">
                <div class="text-xs text-light-muted mb-2 uppercase tracking-wider" id="result-label-3">{{ __($t . '.recipient_gets') }}</div>
    ...
    <div class="bg-darkBg rounded-lg p-4 border border-gold/10 mb-4">
        ...
        <div class="space-y-2 text-sm">
            <div class="flex justify-between">
                <span class="text-light-muted">{{ __($t . '.fee_type') }}</span>
                <span class="text-light font-mono" id="breakdown-fee-type">{{ __($t . '.free') }}</span>
            </div>
            <div class="flex justify-between">
                <span class="text-light-muted">{{ __($t . '.percentage_fee') }} (<span id="breakdown-percent">0%</span>)</span>
            ...
            <div id="breakdown-fixed-row" class="hidden flex justify-between">
                <span class="text-light-muted">{{ __($t . '.fixed_fee') }}</span>
            ...
            <div class="border-t border-gold/10 pt-2 flex justify-between font-semibold">
                <span class="text-light">{{ __($t . '.total_venmo_fee') }}</span>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-left text-light-muted border-b border-gold/10">
                        <th class="pb-2 pr-4">{{ __($t . '.amount') }}</th>
                        <th class="pb-2 pr-4">{{ __($t . '.venmo_fee') }}</th>
                        <th class="pb-2 pr-4">{{ __($t . '.recipient_gets') }}</th>
                        <th class="pb-2">{{ __($t . '.effective_rate') }}</th>
                    </tr>
                </thead>
...
<div id="stats-bar" class="hidden mt-4 flex flex-wrap gap-4 text-xs text-light-muted">
    <span>{{ __($t . '.type_stat') }}: <span id="stat-type" class="text-gold">-</span></span>
    <span>{{ __($t . '.rate_stat') }}: <span id="stat-rate" class="text-gold">-</span></span>
    <span>{{ __($t . '.mode_stat') }}: <span id="stat-mode" class="text-gold">-</span></span>
</div>
