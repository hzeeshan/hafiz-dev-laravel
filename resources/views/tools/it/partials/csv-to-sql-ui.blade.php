...
<div class="mb-6 p-4 bg-darkBg rounded-lg border border-gold/10">
    <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        <div>
            <label for="table-name" class="text-light font-semibold mb-2 block text-sm">{{ __($t . '.table_name') }}</label>
        ...
        <div>
            <label for="sql-dialect" class="text-light font-semibold mb-2 block text-sm">{{ __($t . '.sql_dialect') }}</label>
        ...
        <div>
            ...
            <select id="delimiter" class="w-full bg-darkCard border border-gold/20 rounded-lg px-3 py-2 text-light text-sm focus:border-gold/50 focus:outline-none cursor-pointer">
                ...
                <option value="\t">{{ __($t . '.delimiter_tab') }}</option>
                <option value="|">{{ __($t . '.delimiter_pipe') }}</option>
            </select>
        </div>
        <div>
            <label for="insert-mode" class="text-light font-semibold mb-2 block text-sm">{{ __($t . '.insert_mode') }}</label>
            <select id="insert-mode" class="w-full bg-darkCard border border-gold/20 rounded-lg px-3 py-2 text-light text-sm focus:border-gold/50 focus:outline-none cursor-pointer">
                <option value="single">{{ __($t . '.insert_single') }}</option>
                <option value="batch">{{ __($t . '.insert_batch') }}</option>
            </select>
        </div>
    </div>
    <div class="flex flex-wrap gap-6">
        <label class="flex items-center gap-2 cursor-pointer">
            ...
            <span class="text-sm text-light-muted">{{ __($t . '.include_create') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
            ...
            <span class="text-sm text-light-muted">{{ __($t . '.drop_table') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
            ...
            <span class="text-sm text-light-muted">{{ __($t . '.wrap_transaction') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
            ...
            <span class="text-sm text-light-muted">{{ __($t . '.empty_as_null') }}</span>
        </label>
    </div>
</div>
...
3,Bob Wilson,bob@example.com,45,105000,false"
    ...
    <div class="flex flex-col">
        ...
        <textarea
            ...
            placeholder="{{ __($t . '.sql_output_placeholder') }}"
            readonly
        ></textarea>
    </div>
</div>
...
<div id="stats-bar" class="hidden bg-darkBg rounded-lg p-4 border border-gold/10">
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div class="text-center">
            ...
            <div class="text-light-muted text-sm">{{ __($t . '.statements') }}</div>
        </div>
        <div class="text-center">
            <div id="stat-dialect" class="text-2xl font-bold text-light mb-1">—</div>
            <div class="text-light-muted text-sm">{{ __($t . '.dialect') }}</div>
        </div>
        <div class="text-center">
