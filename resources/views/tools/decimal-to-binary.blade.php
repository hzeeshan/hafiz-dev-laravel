    ...
    (function() {
        ...
        function buildDivisionBreakdown(dec) {
            ...
            if (n === 0) {
                ...
                breakdownResult.innerHTML = 'Read remainders bottom → top: <span class="text-gold font-mono font-bold">0</span>';
                return;
            }
            const steps = [];
            let step = 1;
            ...
            breakdownResult.innerHTML = `Read remainders bottom → top: <span class="text-gold font-mono font-bold">${binary}</span>`;
        }
        ...
        function convert() {
            ...
            if (isDecToBin) {
                ...
                showSuccess(`Converted ${values.length} decimal value${values.length > 1 ? 's' : ''} to binary`);
            } else {
                for (const val of values) {
                    const dec = binToDec(val);
                    if (dec === null) {
                        showError(`Invalid binary value: "${val}" (digits must be 0 or 1)`);
                        return;
                    }
                    results.push(dec);
                }
                outputEl.value = results.join('\n');
                breakdown.classList.add('hidden');
                showSuccess(`Converted ${values.length} binary value${values.length > 1 ? 's' : ''} to decimal`);
            }
        }
        ...
        function swapDirection() {
            ...
            if (isDecToBin) {
                directionLabel.innerHTML = '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"></path></svg> Decimal → Binary';
                inputLabel.textContent = 'Decimal Input';
                outputLabel.textContent = 'Binary Output';
                inputEl.placeholder = 'Enter decimal numbers (e.g., 42, 255, 1024)...';
                outputEl.placeholder = 'Binary result will appear here...';
            } else {
                directionLabel.innerHTML = '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 17l-5-5m0 0l5-5m-5 5h12"></path></svg> Binary → Decimal';
                inputLabel.textContent = 'Binary Input';
                outputLabel.textContent = 'Decimal Output';
                inputEl.placeholder = 'Enter binary numbers (e.g., 101010, 11111111)...';
        ...
        btnSample.addEventListener('click', function() {
            inputEl.value = isDecToBin ? '42, 255, 1024, 65535' : '101010, 11111111, 10000000000';
            outputEl.value = '';
            breakdown.classList.add('hidden');
            convert();
        });
