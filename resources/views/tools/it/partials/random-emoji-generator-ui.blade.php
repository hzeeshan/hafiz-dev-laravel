...
<div class="mb-6 p-4 bg-darkBg rounded-lg border border-gold/10">
    <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
            <label for="emoji-category" class="text-light font-semibold mb-2 block text-sm">{{ __($t . '.category') }}</label>
            <select id="emoji-category" class="w-full bg-darkCard border border-gold/20 rounded-lg px-3 py-2 text-light text-sm focus:border-gold/50 focus:outline-none cursor-pointer">
                <option value="all">{{ __($t . '.cat_all') }}</option>
                <option value="smileys">{{ __($t . '.cat_smileys') }}</option>
                <option value="animals">{{ __($t . '.cat_animals') }}</option>
                <option value="food">{{ __($t . '.cat_food') }}</option>
                <option value="activities">{{ __($t . '.cat_activities') }}</option>
                <option value="travel">{{ __($t . '.cat_travel') }}</option>
                <option value="objects">{{ __($t . '.cat_objects') }}</option>
                <option value="symbols">{{ __($t . '.cat_symbols') }}</option>
                <option value="flags">{{ __($t . '.cat_flags') }}</option>
            </select>
        </div>
        <div>
            <label for="emoji-count" class="text-light font-semibold mb-2 block text-sm">{{ __($t . '.quantity') }}</label>
        ...
        <div>
            <label for="emoji-separator" class="text-light font-semibold mb-2 block text-sm">{{ __($t . '.separator') }}</label>
            <select id="emoji-separator" class="w-full bg-darkCard border border-gold/20 rounded-lg px-3 py-2 text-light text-sm focus:border-gold/50 focus:outline-none cursor-pointer">
                <option value="none">{{ __($t . '.sep_none') }}</option>
                <option value="space" selected>{{ __($t . '.sep_space') }}</option>
                <option value="newline">{{ __($t . '.sep_newline') }}</option>
                <option value="comma">{{ __($t . '.sep_comma') }}</option>
            </select>
        </div>
        <div class="flex items-end pb-2">
            <label class="flex items-center gap-2 cursor-pointer">
                ...
                <span class="text-sm text-light-muted">{{ __($t . '.unique_only') }}</span>
            </label>
        </div>
    </div>
</div>
...
<div id="stats-bar" class="hidden bg-darkBg rounded-lg p-4 border border-gold/10 mb-6">
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="text-center">
            ...
            <div class="text-light-muted text-sm">{{ __($t . '.stat_emojis_generated') }}</div>
        </div>
        <div class="text-center">
            <div id="stat-category" class="text-2xl font-bold text-light mb-1">-</div>
            <div class="text-light-muted text-sm">{{ __($t . '.stat_category') }}</div>
        </div>
        <div class="text-center">
            <div id="stat-unique" class="text-2xl font-bold text-light mb-1">0</div>
            <div class="text-light-muted text-sm">{{ __($t . '.stat_unique') }}</div>
        </div>
        <div class="text-center">
            <div id="stat-pool" class="text-2xl font-bold text-light mb-1">0</div>
            <div class="text-light-muted text-sm">{{ __($t . '.stat_pool') }}</div>
        </div>
    </div>
</div>
