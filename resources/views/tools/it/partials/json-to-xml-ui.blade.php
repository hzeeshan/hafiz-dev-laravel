...
<div class="mb-6 p-4 bg-darkBg rounded-lg border border-gold/10">
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
        ...
        <div>
            <label for="array-item-name" class="text-light font-semibold mb-2 block text-sm">{{ __($t . '.array_item_name') }}</label>
            <select id="array-item-name" class="w-full bg-darkCard border border-gold/20 rounded-lg px-3 py-2 text-light text-sm focus:border-gold/50 focus:outline-none cursor-pointer">
                <option value="auto">{{ __($t . '.auto_singularize') }}</option>
                <option value="item">{{ __($t . '.always_item') }}</option>
            </select>
        </div>
        <div>
            ...
            <select id="indent-size" class="w-full bg-darkCard border border-gold/20 rounded-lg px-3 py-2 text-light text-sm focus:border-gold/50 focus:outline-none cursor-pointer">
                <option value="2" selected>{{ __($t . '.two_spaces') }}</option>
                <option value="4">{{ __($t . '.four_spaces') }}</option>
                <option value="tab">Tab</option>
                <option value="0">{{ __($t . '.minified') }}</option>
            </select>
        </div>
    </div>
    <div class="flex flex-wrap gap-6">
        ...
        <label class="flex items-center gap-2 cursor-pointer">
            ...
            <span class="text-sm text-light-muted">{{ __($t . '.add_type_attributes') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
            ...
            <span class="text-sm text-light-muted">{{ __($t . '.self_close_empty') }}</span>
        </label>
    </div>
</div>
...
<div class="grid lg:grid-cols-2 gap-4 mb-6">
    <div class="flex flex-col">
        ...
        <textarea
            ...
            placeholder="{{ __($t . '.json_placeholder') }}"
            spellcheck="false"
        ></textarea>
    </div>
...
<div id="stats-bar" class="hidden bg-darkBg rounded-lg p-4 border border-gold/10">
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="text-center">
            ...
            <div class="text-light-muted text-sm">{{ __($t . '.xml_elements') }}</div>
        </div>
        <div class="text-center">
