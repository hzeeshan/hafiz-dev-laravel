<x-layout>
    ...
    (function() {
        ...
        function buildSocialLinks(v, font, color) {
            const links = [];
            if (v.linkedin) links.push(`<a href="${esc(v.linkedin)}" style="color:${color};text-decoration:none;font-family:${font};font-size:12px;" target="_blank">LinkedIn</a>`);
            if (v.github) links.push(`<a href="${esc(v.github)}" style="color:${color};text-decoration:none;font-family:${font};font-size:12px;" target="_blank">GitHub</a>`);
            if (v.twitter) links.push(`<a href="${esc(v.twitter)}" style="color:${color};text-decoration:none;font-family:${font};font-size:12px;" target="_blank">Twitter/X</a>`);
            if (v.website) links.push(`<a href="${esc(v.website)}" style="color:${color};text-decoration:none;font-family:${font};font-size:12px;" target="_blank">Portfolio</a>`);
            return links.join(' &nbsp;|&nbsp; ');
        }
        ...
        function render() {
            const v = getValues();
            if (!v.name) { preview.innerHTML = '<p style="color:#999;font-style:italic;font-size:14px;">Enter your name to see a preview...</p>'; return; }
        ...
        btnCopyHtml.addEventListener('click', function() {
            ...
            showSuccess('HTML signature copied — paste into your email client');
        });
        ...
        btnCopySource.addEventListener('click', function() {
            navigator.clipboard.writeText(getSignatureHtml()).then(() => {
                ...
                this.innerHTML = '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Copied!';
                setTimeout(() => { this.innerHTML = orig; }, 2000);
                showSuccess('HTML source code copied');
            });
        });
