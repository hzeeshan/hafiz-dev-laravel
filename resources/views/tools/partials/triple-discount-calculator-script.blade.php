...
(function() {
    ...
    var inputDiscount3 = document.getElementById('input-discount3');
    ...
    function calculate() {
        ...
        var d3 = parseFloat(inputDiscount3.value) || 0;
        ...
        var d3Amount = afterD2 * (d3 / 100);
        var afterD3 = afterD2 - d3Amount;

        // Tax
        var taxAmount = afterD3 * (taxRate / 100);
        var finalPrice = afterD3 + taxAmount;

        // Combined discount percentage
        var combinedPct = (1 - (1 - d1 / 100) * (1 - d2 / 100) * (1 - d3 / 100)) * 100;
        var totalSaved = d1Amount + d2Amount + d3Amount;

        // Naive (simple addition)
        var naivePct = d1 + d2 + d3;
        var naivePrice = price * (1 - naivePct / 100);
        var stackingPenalty = afterD3 - naivePrice;
        ...
        if (taxRate > 0) {
            ...
            document.getElementById('result-final').textContent = formatMoney(afterD3, sym);
        }
        ...
        document.getElementById('breakdown-d3-pct').textContent = d3 + '%';
        document.getElementById('breakdown-d3-amount').textContent = '-' + formatMoney(d3Amount, sym);
        document.getElementById('breakdown-after-d3').textContent = formatMoney(afterD3, sym);
    ...
    function getResultText() {
        ...
        var text = 'Triple Discount Calculation\n' +
            '============================\n' +
            ...
            'After Second Discount:   ' + document.getElementById('breakdown-after-d2').textContent + '\n' +
            'Third Discount (' + d3 + '%):   ' + document.getElementById('breakdown-d3-amount').textContent + '\n' +
            'After All Discounts:     ' + document.getElementById('breakdown-after-d3').textContent + '\n';
        ...
        text += '\nSummary:\n' +
            ...
            '  Naive Sum (' + d1 + '+' + d2 + '+' + d3 + '): ' + document.getElementById('summary-naive-pct').textContent + '\n' +
            '  Stacking Penalty:  ' + document.getElementById('summary-difference').textContent + '\n' +
            '\nCalculated at hafiz.dev/tools/triple-discount-calculator';
    ...
    btnDownload.addEventListener('click', function() {
        ...
        a.href = url;
        a.download = 'triple-discount-calculation.txt';
        a.click();
        URL.revokeObjectURL(url);
        showSuccess('Downloaded triple-discount-calculation.txt');
    });
    ...
    [inputPrice, inputDiscount1, inputDiscount2, inputDiscount3, taxInput].forEach(function(el) {
