...
<div class="flex flex-wrap items-center gap-4 mb-4 p-3 rounded-lg bg-darkBg border border-gold/10">
    <div class="flex items-center gap-2">
        <label for="repeat-count" class="text-light-muted text-sm">{{ __($t . '.repeat_label') }}</label>
        ...
        <span class="text-light-muted text-sm">{{ __($t . '.times') }}</span>
    </div>

    <div class="h-8 w-px bg-gold/20 hidden sm:block"></div>

    <div class="flex items-center gap-2">
        <label for="separator-select" class="text-light-muted text-sm">{{ __($t . '.separator_label') }}</label>
        <select id="separator-select" class="bg-darkBg border border-gold/30 text-light rounded-lg px-3 py-2 text-sm focus:border-gold focus:outline-none">
            <option value="newline">{{ __($t . '.newline') }}</option>
            <option value="space">{{ __($t . '.space') }}</option>
            <option value="comma">{{ __($t . '.comma') }}</option>
            <option value="semicolon">{{ __($t . '.semicolon') }}</option>
            <option value="tab">{{ __($t . '.tab') }}</option>
            <option value="custom">{{ __($t . '.custom') }}</option>
        </select>
    </div>

    <div id="custom-separator-wrapper" class="hidden flex items-center gap-2">
        <label for="custom-separator" class="text-light-muted text-sm">{{ __($t . '.custom_label') }}</label>
        <input type="text" id="custom-separator" placeholder="{{ __($t . '.custom_placeholder') }}" class="w-24 bg-darkBg border border-gold/30 text-light rounded-lg px-3 py-2 text-sm focus:border-gold focus:outline-none">
    </div>
    ...
    <label class="flex items-center gap-2 cursor-pointer">
        ...
        <span class="text-light-muted text-sm">{{ __($t . '.add_numbering') }}</span>
    </label>
</div>
...
<div id="stats-bar" class="hidden grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
    <div class="bg-darkBg rounded-lg p-4 border border-gold/10 text-center">
        ...
        <div class="text-light-muted text-sm">{{ __($t . '.stat_repetitions') }}</div>
    </div>

    <div class="bg-darkBg rounded-lg p-4 border border-gold/10 text-center">
        <div id="stat-total-chars" class="text-2xl font-bold text-gold mb-1">0</div>
        <div class="text-light-muted text-sm">{{ __($t . '.stat_total_chars') }}</div>
    </div>

    <div class="bg-darkBg rounded-lg p-4 border border-gold/10 text-center">
        <div id="stat-total-lines" class="text-2xl font-bold text-gold mb-1">0</div>
        <div class="text-light-muted text-sm">{{ __($t . '.stat_total_lines') }}</div>
    </div>

    <div class="bg-darkBg rounded-lg p-4 border border-gold/10 text-center">
        <div id="stat-output-size" class="text-2xl font-bold text-gold mb-1">0 B</div>
        <div class="text-light-muted text-sm">{{ __($t . '.stat_output_size') }}</div>
    </div>
</div>
