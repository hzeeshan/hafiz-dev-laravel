...
<div class="mb-6 p-4 bg-darkBg rounded-lg border border-gold/10">
    <div class="text-sm text-light-muted mb-3 font-medium">{{ __($t . '.font_style') }}</div>
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
        <label class="flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg border border-gold/10 hover:border-gold/30 transition-colors has-[:checked]:border-gold has-[:checked]:bg-gold/10">
            ...
            <span class="text-sm text-light">{{ __($t . '.bold') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg border border-gold/10 hover:border-gold/30 transition-colors has-[:checked]:border-gold has-[:checked]:bg-gold/10">
            ...
            <span class="text-sm text-light">{{ __($t . '.italic') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg border border-gold/10 hover:border-gold/30 transition-colors has-[:checked]:border-gold has-[:checked]:bg-gold/10">
            ...
            <span class="text-sm text-light">{{ __($t . '.bold_italic') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg border border-gold/10 hover:border-gold/30 transition-colors has-[:checked]:border-gold has-[:checked]:bg-gold/10">
            ...
            <span class="text-sm text-light">{{ __($t . '.script') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg border border-gold/10 hover:border-gold/30 transition-colors has-[:checked]:border-gold has-[:checked]:bg-gold/10">
            ...
            <span class="text-sm text-light">{{ __($t . '.script_bold') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg border border-gold/10 hover:border-gold/30 transition-colors has-[:checked]:border-gold has-[:checked]:bg-gold/10">
            ...
            <span class="text-sm text-light">{{ __($t . '.monospace') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg border border-gold/10 hover:border-gold/30 transition-colors has-[:checked]:border-gold has-[:checked]:bg-gold/10">
            ...
            <span class="text-sm text-light">{{ __($t . '.double_struck') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg border border-gold/10 hover:border-gold/30 transition-colors has-[:checked]:border-gold has-[:checked]:bg-gold/10">
            ...
            <span class="text-sm text-light">{{ __($t . '.fraktur') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg border border-gold/10 hover:border-gold/30 transition-colors has-[:checked]:border-gold has-[:checked]:bg-gold/10">
            ...
            <span class="text-sm text-light">{{ __($t . '.sans_serif') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg border border-gold/10 hover:border-gold/30 transition-colors has-[:checked]:border-gold has-[:checked]:bg-gold/10">
            ...
            <span class="text-sm text-light">{{ __($t . '.sans_bold') }}</span>
        </label>
    </div>
</div>
...
<div id="stats-bar" class="hidden bg-darkBg rounded-lg p-4 border border-gold/10">
    <div class="grid grid-cols-3 gap-4">
        <div class="text-center">
            ...
            <div class="text-light-muted text-sm">{{ __($t . '.stat_chars') }}</div>
        </div>
        <div class="text-center">
            <div id="stat-converted" class="text-2xl font-bold text-light mb-1">0</div>
            <div class="text-light-muted text-sm">{{ __($t . '.stat_converted') }}</div>
        </div>
        <div class="text-center">
            <div id="stat-unchanged" class="text-2xl font-bold text-light mb-1">0</div>
            <div class="text-light-muted text-sm">{{ __($t . '.stat_unchanged') }}</div>
        </div>
    </div>
</div>
...
<div class="mt-6 pt-6 border-t border-gold/10">
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-3" id="font-preview-grid">
        <div class="bg-darkBg rounded-lg p-3 border border-gold/10 cursor-pointer hover:border-gold/30 transition-colors" data-style="bold">
            <div class="text-xs text-light-muted mb-1 uppercase tracking-wider">{{ __($t . '.preview_bold') }}</div>
            <div id="preview-bold" class="text-light font-mono text-sm break-all"></div>
        </div>
        <div class="bg-darkBg rounded-lg p-3 border border-gold/10 cursor-pointer hover:border-gold/30 transition-colors" data-style="italic">
            <div class="text-xs text-light-muted mb-1 uppercase tracking-wider">{{ __($t . '.preview_italic') }}</div>
            <div id="preview-italic" class="text-light font-mono text-sm break-all"></div>
        </div>
        <div class="bg-darkBg rounded-lg p-3 border border-gold/10 cursor-pointer hover:border-gold/30 transition-colors" data-style="bold-italic">
            <div class="text-xs text-light-muted mb-1 uppercase tracking-wider">{{ __($t . '.preview_bold_italic') }}</div>
            <div id="preview-bold-italic" class="text-light font-mono text-sm break-all"></div>
        </div>
        <div class="bg-darkBg rounded-lg p-3 border border-gold/10 cursor-pointer hover:border-gold/30 transition-colors" data-style="script">
            <div class="text-xs text-light-muted mb-1 uppercase tracking-wider">{{ __($t . '.preview_script') }}</div>
            <div id="preview-script" class="text-light font-mono text-sm break-all"></div>
        </div>
        <div class="bg-darkBg rounded-lg p-3 border border-gold/10 cursor-pointer hover:border-gold/30 transition-colors" data-style="script-bold">
            <div class="text-xs text-light-muted mb-1 uppercase tracking-wider">{{ __($t . '.preview_script_bold') }}</div>
            <div id="preview-script-bold" class="text-light font-mono text-sm break-all"></div>
        </div>
        <div class="bg-darkBg rounded-lg p-3 border border-gold/10 cursor-pointer hover:border-gold/30 transition-colors" data-style="monospace">
            <div class="text-xs text-light-muted mb-1 uppercase tracking-wider">{{ __($t . '.preview_monospace') }}</div>
            <div id="preview-monospace" class="text-light font-mono text-sm break-all"></div>
        </div>
        <div class="bg-darkBg rounded-lg p-3 border border-gold/10 cursor-pointer hover:border-gold/30 transition-colors" data-style="double-struck">
            <div class="text-xs text-light-muted mb-1 uppercase tracking-wider">{{ __($t . '.preview_double_struck') }}</div>
            <div id="preview-double-struck" class="text-light font-mono text-sm break-all"></div>
        </div>
        <div class="bg-darkBg rounded-lg p-3 border border-gold/10 cursor-pointer hover:border-gold/30 transition-colors" data-style="fraktur">
            <div class="text-xs text-light-muted mb-1 uppercase tracking-wider">{{ __($t . '.preview_fraktur') }}</div>
            <div id="preview-fraktur" class="text-light font-mono text-sm break-all"></div>
        </div>
        <div class="bg-darkBg rounded-lg p-3 border border-gold/10 cursor-pointer hover:border-gold/30 transition-colors" data-style="sans-serif">
            <div class="text-xs text-light-muted mb-1 uppercase tracking-wider">{{ __($t . '.preview_sans_serif') }}</div>
            <div id="preview-sans-serif" class="text-light font-mono text-sm break-all"></div>
        </div>
        <div class="bg-darkBg rounded-lg p-3 border border-gold/10 cursor-pointer hover:border-gold/30 transition-colors" data-style="sans-bold">
            <div class="text-xs text-light-muted mb-1 uppercase tracking-wider">{{ __($t . '.preview_sans_bold') }}</div>
            <div id="preview-sans-bold" class="text-light font-mono text-sm break-all"></div>
        </div>
    </div>
</div>
