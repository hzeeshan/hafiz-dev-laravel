...
<div class="mb-6 p-4 bg-darkBg rounded-lg border border-gold/10">
    <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
            <label for="encoding" class="text-light font-semibold mb-2 block text-sm">{{ __($t . '.encoding') }}</label>
        ...
        <div>
            ...
            <select id="separator" class="w-full bg-darkCard border border-gold/20 rounded-lg px-3 py-2 text-light text-sm focus:border-gold/50 focus:outline-none cursor-pointer">
                <option value=" ">{{ __($t . '.space') }}</option>
                <option value="  ">{{ __($t . '.double_space') }}</option>
                <option value="\n">{{ __($t . '.new_line') }}</option>
        ...
        <div class="flex items-end pb-2">
            <label class="flex items-center gap-2 cursor-pointer">
                ...
                <span class="text-sm text-light-muted">{{ __($t . '.add_prefix') }}</span>
            </label>
        </div>
    </div>
</div>
...
<div class="grid lg:grid-cols-2 gap-4 mb-6">
    ...
    <div class="flex flex-col">
        <label for="text-input" class="text-light font-semibold mb-2 flex items-center gap-2">
            ...
            <span id="input-label">{{ __($t . '.text_input') }}</span>
        </label>
        <textarea
            id="text-input"
...
<div id="stats-bar" class="hidden bg-darkBg rounded-lg p-4 border border-gold/10 mb-6">
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="text-center">
            ...
            <div class="text-light-muted text-sm">{{ __($t . '.bytes') }}</div>
        </div>
        <div class="text-center">
            <div id="stat-bits" class="text-2xl font-bold text-light mb-1">0</div>
            <div class="text-light-muted text-sm">{{ __($t . '.total_bits') }}</div>
        </div>
        <div class="text-center">
            <div id="stat-encoding" class="text-2xl font-bold text-light mb-1">—</div>
            <div class="text-light-muted text-sm">{{ __($t . '.encoding_label') }}</div>
        </div>
    </div>
</div>
...
<div id="breakdown" class="hidden bg-darkBg rounded-lg p-4 border border-gold/10">
    <h3 class="text-light font-semibold mb-3 flex items-center gap-2 text-sm">
        ...
        {{ __($t . '.character_breakdown') }}
        <span class="text-light-muted font-normal">{{ __($t . '.first_50') }}</span>
    </h3>
    <div class="overflow-x-auto border border-gold/10 rounded-lg">
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-darkCard border-b border-gold/10">
                    <th class="px-3 py-2 text-left text-xs font-semibold text-gold uppercase">{{ __($t . '.char') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-semibold text-gold uppercase">{{ __($t . '.decimal') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-semibold text-gold uppercase">{{ __($t . '.hex') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-semibold text-gold uppercase">{{ __($t . '.octal') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-semibold text-gold uppercase">{{ __($t . '.binary') }}</th>
                </tr>
            </thead>
