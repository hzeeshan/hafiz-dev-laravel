...
<div class="grid lg:grid-cols-2 gap-4 mb-6">
    <div class="flex flex-col">
        ...
        <textarea
            ...
            placeholder="{{ __($t . '.js_strings.input_placeholder') }}"
            spellcheck="false"
        ></textarea>
    </div>
    ...
    <div class="flex flex-col">
        <div class="flex items-center justify-between mb-2">
            ...
            <div class="flex gap-1 bg-darkBg rounded-lg p-0.5 border border-gold/10">
                <button id="btn-view-preview" class="px-3 py-1 text-xs rounded-md bg-gold/20 text-gold font-medium transition-all cursor-pointer">{{ __($t . '.preview') }}</button>
                <button id="btn-view-source" class="px-3 py-1 text-xs rounded-md text-light-muted hover:text-light transition-all cursor-pointer">{{ __($t . '.source') }}</button>
            </div>
        </div>
...
<div id="stats-bar" class="hidden bg-darkBg rounded-lg p-4 border border-gold/10">
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="text-center">
            ...
            <div class="text-light-muted text-sm">{{ __($t . '.html_elements') }}</div>
        </div>
        <div class="text-center">
            <div id="stat-headings" class="text-2xl font-bold text-light mb-1">0</div>
            <div class="text-light-muted text-sm">{{ __($t . '.headings') }}</div>
        </div>
        <div class="text-center">
            <div id="stat-links" class="text-2xl font-bold text-light mb-1">0</div>
            <div class="text-light-muted text-sm">{{ __($t . '.links') }}</div>
        </div>
        <div class="text-center">
