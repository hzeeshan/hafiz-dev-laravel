...
const SLUG = 'playback-speed-calculator';

test.describe('Playback Speed Calculator — functional tests', () => {
  test.beforeEach(async ({ toolPage }) => {
    await toolPage.goto(SLUG);
  });

  test('calculates watch times for valid input', async ({ page }) => {
    await page.fill('#input-hours', '2');
    ...
    // Check custom result displays something meaningful
  ...
  test('sample button loads data and calculates', async ({ page }) => {
    ...
    // Default media type is "video" (22 min sample), check minutes got populated
    const minutes = await page.inputValue('#input-minutes');
  ...
  test('speed table shows all preset speeds', async ({ page }) => {
    await page.fill('#input-hours', '1');
    await page.click('#btn-calculate');

    // Should have rows for 0.75x, 1x, 1.25x, 1.5x, 1.75x, 2x, 2.5x, 3x
  ...
  test('media type buttons toggle correctly', async ({ page }) => {
    const podcastBtn = page.locator('[data-media="podcast"]');
    await podcastBtn.click();

    // Podcast button should now have gold background
    await expect(podcastBtn).toHaveClass(/bg-gold/);

    // Video button should no longer have gold background
    const videoBtn = page.locator('[data-media="video"]');
    await expect(videoBtn).not.toHaveClass(/bg-gold/);
  });

  test('multiple items multiplies duration', async ({ page }) => {
    await page.fill('#input-hours', '1');
    await page.fill('#input-minutes', '0');
    await page.fill('#input-seconds', '0');
    await page.fill('#input-items', '3');
    await page.click('#btn-calculate');

    // Original length should show 3h (3 items x 1h each)
    const statOriginal = await page.textContent('#stat-original');
    expect(statOriginal).toContain('3h');
  });
