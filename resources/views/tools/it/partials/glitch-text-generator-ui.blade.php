...
<div class="mb-6 p-4 bg-darkBg rounded-lg border border-gold/10">
    <div class="grid sm:grid-cols-2 gap-6">
        {{-- Style Selector --}}
        <div>
            <label class="text-light font-semibold mb-3 block text-sm">{{ __($t . '.glitch_style') }}</label>
            <select id="glitch-style" class="w-full bg-darkBg border border-gold/20 rounded-lg p-2.5 text-sm text-light focus:border-gold focus:outline-none focus:ring-1 focus:ring-gold/30 cursor-pointer">
                <option value="zalgo">{{ __($t . '.style_zalgo') }}</option>
                <option value="strike">{{ __($t . '.style_strike') }}</option>
                <option value="matrix">{{ __($t . '.style_matrix') }}</option>
                <option value="vaporwave">{{ __($t . '.style_vaporwave') }}</option>
                <option value="binary">{{ __($t . '.style_binary') }}</option>
            </select>
        </div>
        {{-- Intensity --}}
        <div>
            <label class="text-light font-semibold mb-3 block text-sm">{{ __($t . '.intensity') }}</label>
            ...
            <div class="flex justify-between text-xs text-light-muted mt-1">
                <span>{{ __($t . '.subtle') }}</span>
                <span id="intensity-label" class="text-gold font-semibold">{{ __($t . '.medium') }}</span>
                <span>{{ __($t . '.extreme') }}</span>
            </div>
        </div>
    </div>
</div>
...
<div id="stats-bar" class="hidden bg-darkBg rounded-lg p-4 border border-gold/10">
    <div class="grid grid-cols-3 gap-4">
        ...
        <div class="text-center">
            ...
            <div class="text-light-muted text-sm">{{ __($t . '.style') }}</div>
        </div>
    </div>
</div>
