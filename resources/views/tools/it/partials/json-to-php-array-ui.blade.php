...
<div class="mb-6 p-4 bg-darkBg rounded-lg border border-gold/10">
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
        <div>
            <label for="variable-name" class="text-light font-semibold mb-2 block text-sm">{{ __($t . '.variable_name') }}</label>
        ...
        <div>
            <label for="array-style" class="text-light font-semibold mb-2 block text-sm">{{ __($t . '.array_syntax') }}</label>
            <select id="array-style" class="w-full bg-darkCard border border-gold/20 rounded-lg px-3 py-2 text-light text-sm focus:border-gold/50 focus:outline-none cursor-pointer">
                <option value="short" selected>{{ __($t . '.short_syntax') }}</option>
                <option value="long">{{ __($t . '.long_syntax') }}</option>
            </select>
        </div>
        <div>
            ...
            <select id="indent-size" class="w-full bg-darkCard border border-gold/20 rounded-lg px-3 py-2 text-light text-sm focus:border-gold/50 focus:outline-none cursor-pointer">
                <option value="2">{{ __($t . '.two_spaces') }}</option>
                <option value="4" selected>{{ __($t . '.four_spaces') }}</option>
    ...
    <div class="flex flex-wrap gap-6">
        <label class="flex items-center gap-2 cursor-pointer">
            ...
            <span class="text-sm text-light-muted">{{ __($t . '.trailing_commas') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
            ...
            <span class="text-sm text-light-muted">{{ __($t . '.variable_assignment') }}</span>
        </label>
        <label class="flex items-center gap-2 cursor-pointer">
            ...
            <span class="text-sm text-light-muted">{{ __($t . '.single_quotes') }}</span>
        </label>
    </div>
</div>
...
<div class="grid lg:grid-cols-2 gap-4 mb-6">
    <div class="flex flex-col">
        ...
        <textarea
            ...
            placeholder="{{ __($t . '.paste_placeholder') }}"
            spellcheck="false"
        ></textarea>
    </div>
...
<div id="stats-bar" class="hidden bg-darkBg rounded-lg p-4 border border-gold/10">
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="text-center">
            ...
            <div class="text-light-muted text-sm">{{ __($t . '.stat_keys') }}</div>
        </div>
        <div class="text-center">
            <div id="stat-depth" class="text-2xl font-bold text-light mb-1">0</div>
            <div class="text-light-muted text-sm">{{ __($t . '.stat_depth') }}</div>
        </div>
        <div class="text-center">
            <div id="stat-arrays" class="text-2xl font-bold text-light mb-1">0</div>
            <div class="text-light-muted text-sm">{{ __($t . '.stat_arrays') }}</div>
        </div>
        <div class="text-center">
            <div id="stat-lines" class="text-2xl font-bold text-light mb-1">0</div>
            <div class="text-light-muted text-sm">{{ __($t . '.stat_lines') }}</div>
        </div>
    </div>
</div>
